/**
 * ═══════════════════════════════════════════════════════════════════════
 *                  JULESMOTOR - PARITY CORRECTOR V2
 * ═══════════════════════════════════════════════════════════════════════
 *
 * Algorithme de correction de la parité Filles/Garçons
 * AMÉLIORATION: Préserve l'équilibrage des scores
 *
 * Version: 2.1.0
 * ═══════════════════════════════════════════════════════════════════════
 */
...
const ParityCorrector = (function(global) {
  ...
  const Validators = global.JulesMotorValidators;
   ...
   * AMÉLIORATION: Priorise les échanges qui préservent les scores
   */
  function findParityCandidates(dataContext, surplusClass, deficitClass, surplusGender, deficitGender, preserveScores = true) {
    ...
    surplusStudents.forEach(studentSurplus => {
      deficitStudents.forEach(studentDeficit => {
        ...
        if (validation.valid) {
          ...
          let impact = calculateParityImpact(studentSurplus, studentDeficit);

          // NOUVEAU: Si preservation des scores activée, bonus pour scores identiques
          let scoreMatchCount = 0;
          if (preserveScores) {
            CRITERES.forEach(critere => {
              const score1 = parseInt(studentSurplus[critere]) || 0;
              const score2 = parseInt(studentDeficit[critere]) || 0;
              if (score1 === score2) {
                scoreMatchCount++;
              }
            });
            ...
            // Si tous identiques: bonus x2, si aucun: bonus x1
            const scorePreservationBonus = 1 + (scoreMatchCount / CRITERES.length);
    ...
    // Trier par impact (qui inclut maintenant la préservation des scores)
   ...
   */
  function findBestParityStrategy(dataContext, class1Data, class2Data, preserveScores = true) {
    ...
    if (delta1 > 0 && delta2 < 0) {
      const candidates = findParityCandidates(dataContext, class1, class2, 'M', 'F', preserveScores);
    ...
    if (delta1 < 0 && delta2 > 0) {
      const candidates = findParityCandidates(dataContext, class1, class2, 'F', 'M', preserveScores);
      if (candidates.length > 0) {
        return {
          type: 'F_M_SWAP',
          class1,
   ...
   * AMÉLIORATION: Préserve l'équilibrage des scores autant que possible
   */
  function correct(dataContext, options = {}) {
    const tolerance = options.tolerance || 1;
    const maxIterations = options.maxIterations || 20;
    const maxCorrections = options.maxCorrections || 50;
    const preserveScores = options.preserveScores !== false; // Activé par défaut

    const Logger = global.Logger || console;

    Logger.log('⚖️  Démarrage correction de parité...');
    if (preserveScores) {
      Logger.log('   Mode: Préservation des scores activée');
    }
        ...
        for (const deficit of imbalances.deficit) {
          ...
          if (candidates.length > 0) {
            ...
            if (applyParityCorrection(best, dataContext)) {
              ...
              const scoreInfo = best.scoreMatchCount !== undefined
                ? ` [${best.scoreMatchCount}/4 scores identiques]`
                : '';

              Logger.log(
                `✅ Parité: ${best.studentSurplus.ID_ELEVE}(M) ↔ ${best.studentDeficit.ID_ELEVE}(F) ` +
                `[${surplus.classe} ↔ ${deficit.classe}]${scoreInfo}`
              );
    ...
    // Statistiques de préservation des scores
    if (preserveScores && allCorrections.length > 0) {
      const avgScoreMatch = allCorrections.reduce((sum, c) => sum + c.scoreMatchCount, 0) / allCorrections.length;
      Logger.log(`  📊 Préservation scores moyenne: ${avgScoreMatch.toFixed(1)}/4 critères`);
    }
