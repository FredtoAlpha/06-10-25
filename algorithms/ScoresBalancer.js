...
const ScoresBalancer = (function(global) {
  ...
  /**
   * Détermine les scores opposés possibles par ordre de priorité
   */
  function getPossibleOppositeScores(score, gaps, targetClass, critere) {
    // 1. Score opposé extrême (priorité max)
    const extremeOpposite = score === 4 ? 1 : (score === 1 ? 4 : null);

    // 2. Scores qui ont un surplus dans la classe cible
    const surplusScores = SCORES.filter(s => {
      const gap = gaps[targetClass][critere][s];
      return gap.surplus > 0 && s !== score;
    }).sort((a, b) => {
      // Trier par surplus décroissant
      return gaps[targetClass][critere][b].surplus - gaps[targetClass][critere][a].surplus;
    });

    // 3. Tous les autres scores
    const allOtherScores = SCORES.filter(s => s !== score);

    // Combiner avec priorité
    const result = [];

    // D'abord l'opposé extrême s'il existe
    if (extremeOpposite && !result.includes(extremeOpposite)) {
      result.push(extremeOpposite);
    }

    // Ensuite les surplus
    surplusScores.forEach(s => {
      if (!result.includes(s)) result.push(s);
    });

    // Enfin tous les autres
    allOtherScores.forEach(s => {
      if (!result.includes(s)) result.push(s);
    });
        ...
        for (const score of prioritizedScores) {
          ...
          for (const surplusClass of surplus) {
            ...
            for (const deficitClass of deficit) {
              ...
              // AMÉLIORATION: Chercher TOUS les scores possibles pour l'échange
              // Ordre de priorité: opposé extrême, puis adjacents, puis tous
              const possibleOppositeScores = getPossibleOppositeScores(score, gaps, deficitClass.classe, critere);

              let swapApplied = false;
              for (const oppositeScore of possibleOppositeScores) {
